...
│   │   └── ContextMenu: 開く / 新しいタブで開く / ─ / ○○で開く▶(動的) / エクスプローラで表示 / パスをコピー / 再読込
...
| **M4** | 「○○で開く」 + 右クリックメニュー充実 | `ExternalEditorLauncher`、ContextMenu の動的構築、エクスプローラで表示 / パスコピー / 再読込 |
...
- `docs/external-editors.md` — 「○○で開く」設定例（サクラエディタ・VS Code・秀丸）
...
8. **「○○で開く」（M4）**: 右クリック → サクラエディタが起動し、対象ファイルが開かれる
